...
class ClientInfoViewModel @Inject constructor(
    ...
    private val service: ClientInfoService
) :
    BaseViewModel(application) {
    private val disposable = CompositeDisposable()

    fun update(clientInfo: ClientInfoDto) {
        disposable.add(
            service.add(clientInfo)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(object : DisposableSingleObserver<SingleResponseModel<ClientInfo>>() {
